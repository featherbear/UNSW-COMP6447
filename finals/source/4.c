/*
    ...
    xml_* functions are all assumed to be safe.
    xml_parse_document takes in a string, and string length, and returns an xml_document.
    xml_document_root returns the root xml_node of the XML document.
    xml_node_child returns the n’th child of an XML node. If no node is found, returns NULL.
    xml_node_children returns the number of children within an xml node.
    xml_string_length is a wrapper around strlen.
    xml_string_copy is a wrapper around strncpy.
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  fread(buf, MAX_FILE_SIZE - 1, sizeof(char), stdin);
